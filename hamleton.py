...
def remove_punctuation(ham):
    ...
    index = 0
    for word in ham:
        ham[index] = word.strip(" ()&[]-—").lower()
        if word == word.upper() and len(word) > 1:
            ham.remove(word)
            index = index - 1
        elif word.lower() in char_names:
            ham.remove(word)
            index = index - 1
        index += 1
    return ham
...
def even_better_random(probdict, tupdict, wordlist, wordnum = 30):
    ...
    allow_rep = 3
    sentence = [random.choice(wordlist).capitalize()]
    while i < wordnum - 1 or ("." not in sentence[-1] and "!" not in sentence[-1] and "?" not in sentence[-1]):
...
def test():
    hamilton = open("All_Lyrics_Speakers").read()
    hamlet = trim_hamlet(open("Hamlet.txt").read())
    ham = hamilton + hamlet
    #ham = trim_hamlet(ham)
    ham = process_hamlet(ham)
    ham = remove_punctuation(ham)
    return even_better_random(prob_array(ham), tuple_array(ham), ham, 30)
