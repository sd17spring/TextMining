...
def even_better_random(probdict, tupdict, wordlist, seedword, wordnum = 30):
    i = 0
    det_rep = 0
    allow_rep = 3
    sentence = [random.choice(wordlist).capitalize()]
...
def test(seedword = "QWERTYUIOP"):
    hamlet_full = open("trump.txt")
    ham = hamlet_full.read()
    #ham = trim_hamlet(ham)
    ham = process_hamlet(ham)
    ham = remove_punctuation(ham)
    return even_better_random(prob_array(ham), tuple_array(ham), ham, seedword, 100)
